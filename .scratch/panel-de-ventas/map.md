...
- ~~**Píxel `1825130408114773` «Pixel Vorare Guatemala»**: disponible, uso publicitario habilitado.~~ **Corregido el 17-ago-2026 contra la documentación vigente de Meta: el píxel NO es el destino de una conversión CTWA.** El destino es un *dataset* creado desde la cuenta de WhatsApp (`POST /{whatsapp_business_account_id}/dataset`, y el `GET` devuelve el que ya exista), al que se postea en `/{dataset_id}/events`. Configurar el píxel publicitario como destino habría mandado los eventos a un lugar real pero equivocado —sin fallar, sin alarma— y nadie lo habría notado hasta preguntarse por qué la pauta no aprende. Por eso la columna del esquema se llama `operations.capi_dataset_id`. La otra mitad que el evento necesita ya existe y no requiere columna: el `business_account_id` de `kapso_connection`, que desde la `0021` es una fila por operación.
